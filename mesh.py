...
def _self_recv_state(cfg, node):
    """#166: this node's own receiver state (a RECV_STATES value) for stamping
    onto outbound presence -- best-effort, NEVER raises into a send path;
    returns None when it can't be determined (no heartbeat yet / not a
    receiver / no config home), which peers read as 'unknown'."""
    try:
        state = _receiver_liveness(cfg, node)[0]
    except (KeyError, OSError, ValueError, TypeError):
        return None
    return state if isinstance(state, str) and state in RECV_STATES else None
...
def _stamp_posture(ctl, recv=None, agent=None, wake=None):
    ...
    if isinstance(recv, str) and recv in RECV_STATES:
        ctl["recv"] = recv
...
def note_peer(cfg, node, via, status=None, posture=None, recv=None, agent=None,
    ...
    if isinstance(recv, str) and recv in RECV_STATES:  # #166: peer's reported receiver-health (untrusted; `[] in set` raises)
