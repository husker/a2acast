...
**Full rationale for every rule below is in [`AGENTS.md`](AGENTS.md).** The rules are repeated here because an instruction you have to go fetch is an instruction that does not get followed.
...
6. **The receive path does not durably mutate trust state.** Writing identity records from an inbound frame is an open design question (#137) — do not cross it in a PR.
7. **Design-first for signed-frame-body changes**, before implementing.

## Review

- **You do not review your own work.** Independent means a different *machine*, not a different session — co-located agents share one node key (#136).
- Verify claims against code. Reproduce; do not trust a summary, including a confident one from another agent.
- Report what you actually ran. "Suite green" is not evidence the specific hazard is covered.
